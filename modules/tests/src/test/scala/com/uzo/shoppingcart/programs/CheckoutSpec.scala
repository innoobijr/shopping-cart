...
import com.uzo.shoppingcart.arbitraries._
import com.uzo.shoppingcart.domain._
...
import com.uzo.suite.PureTestSuite
...
final class CheckoutSpec extends PureTestSuite {
  ...
  forAll { (uid: UserId, pid: PaymentId, oid: OrderId, card: Card) =>
    spec("empty cart") {
      implicit val bg = com.uzo.shoppingcart.background.NoOp
      import com.uzo.shoppingcart.logger.NoOp
  ...
  forAll { (uid: UserId, oid: OrderId, ct: CartTotal, card: Card) =>
    spec("unreachable payment client") {
      Ref.of[IO, List[String]](List.empty).flatMap { logs =>
        implicit val bg     = com.uzo.shoppingcart.background.NoOp
        implicit val logger = com.uzo.shoppingcart.logger.acc(logs)
  ...
  forAll { (uid: UserId, pid: PaymentId, ct: CartTotal, card: Card) =>
    spec("cannot create order, run in the background") {
      Ref.of[IO, Int](0).flatMap { ref =>
        Ref.of[IO, List[String]](List.empty).flatMap { logs =>
          implicit val bg     = com.uzo.shoppingcart.background.counter(ref)
          implicit val logger = com.uzo.shoppingcart.logger.acc(logs)
